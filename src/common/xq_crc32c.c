...
#  include <arm_acle.h>
#  define XQ_CRC32C_HW_ARM 1
#elif defined(__SSE4_2__)
#  include <nmmintrin.h>
#  define XQ_CRC32C_HW_X86 1
#endif

static const uint32_t xq_crc32c_tab[256] = {
...
uint32_t xq_crc32c_portable(uint32_t crc, const void *data, size_t len)
{
    ...
    uint32_t c = ~crc;
    for (; len; len--) c = xq_crc32c_tab[(c ^ *p++) & 0xFFu] ^ (c >> 8);
    return ~c;
}
...
#if defined(XQ_CRC32C_HW_ARM)
    while (len >= 8) { c = __crc32cd(c, xq_ld64le(p)); p += 8; len -= 8; }
    if (len >= 4)    { c = __crc32cw(c, xq_ld32le(p)); p += 4; len -= 4; }
    for (; len; len--) { c = __crc32cb(c, *p++); }
#elif defined(XQ_CRC32C_HW_X86)
    while (len >= 8) { c = (uint32_t)_mm_crc32_u64(c, xq_ld64le(p)); p += 8; len -= 8; }
    if (len >= 4)    { c = _mm_crc32_u32(c, xq_ld32le(p)); p += 4; len -= 4; }
    for (; len; len--) { c = _mm_crc32_u8(c, *p++); }
#else
    (void)0;
    for (; len; len--) c = xq_crc32c_tab[(c ^ *p++) & 0xFFu] ^ (c >> 8);
#endif
...
{
    return xq_crc32c_update(0, data, len);
}

int xq_crc32c_is_accelerated(void)
{
#if defined(XQ_CRC32C_HW_ARM) || defined(XQ_CRC32C_HW_X86)
    return 1;
#else
    return 0;
#endif
